...
// Configuration
var CONFIG = {
    mode: 'unknown', // 'local', 'cloud', or 'unknown'
    lettaServerUrl: null
};
 ...
 */
function detectMode() {
    // Try to detect mode by checking if letta-server is available
    fetch(API_BASE + "/health")
        .then(function(response) { return response.json(); })
        .then(function(data) {
            if (data.letta_server) {
                CONFIG.mode = 'local';
                CONFIG.lettaServerUrl = data.server_url || 'http://letta-server:8283';
                document.getElementById('mode-indicator').textContent = 'Local Mode';
                document.getElementById('mode-indicator').style.backgroundColor = '#4caf50';
            } else {
                CONFIG.mode = 'cloud';
                CONFIG.lettaServerUrl = 'https://api.letta.com';
                document.getElementById('mode-indicator').textContent = 'Cloud Mode';
                document.getElementById('mode-indicator').style.backgroundColor = '#2196f3';
            }
        })
        .catch(function(e) {
            CONFIG.mode = 'unknown';
            document.getElementById('mode-indicator').textContent = 'Unknown Mode';
            document.getElementById('mode-indicator').style.backgroundColor = '#9e9e9e';
        });
}
